...
description: >
  ...
  outreach, and hands results to zoxiro-underwriting and zoxiro-pipeline. Reads
...
absentee-owner pull, etc.) and gives you the file. **For land specifically**, the useful
pulls are different: tax-delinquent and forfeited-land lists, out-of-state or
out-of-county owners (the classic absentee land seller), long-held parcels with no
improvement value on the auditor's card, heirs' and estate parcels, and parcels adjacent
to recent development or new utility extensions. County auditor, treasurer and GIS sites
carry most of this for free, and the land list tools that exist are thinner than the
house ones — so expect Path 2 to do more of the work on land. If they don't have a list tool,
...
- **Filter** to the buy-box: asset type, market/geography, price/value band, and
  ...
  **If `asset_types` includes Land, do not filter land out for having empty house
  fields.** A land record has no beds, baths, sqft, year built or rent, and a filter
  written for houses silently drops every parcel. Filter land on acreage, zoning/use
  code, $/acre against the price band, and the distress flags that actually matter on
  land — tax delinquency above all, which is the single most common reason a parcel is
  cheap. Screen land against `land_margin_*` and `land_max_carry_months`, never against
  the flip margin or a cap rate.
...
- Send the strongest targets to **zoxiro-underwriting** when the user wants the
  numbers run.
...
- **Run the numbers on a sourced deal** → **zoxiro-underwriting**.
