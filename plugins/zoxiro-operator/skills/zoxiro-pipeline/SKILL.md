...
list price, DOM, ARV, est. rehab, margin target %, financed MAO, gap (list − MAO),
status, all-in, rent needed at 1.25x DSCR, market rent comp, source, and notes. The
financed MAO comes from zoxiro-underwriting's model — never fill it with a
rule-of-thumb figure. Write to these columns as they are: never reorder, rename, add or
drop one. On first use (or when the user asks for "the tracker" /
...
**There is a second board template, and it is NOT wired up yet.**
`references/pipeline-board-web.template.html` is the web-publishable variant of the board:
it carries a connector manifest (`{{DRIVE_SERVER_JSON}}`) and reads the queue live, the way
the orchestrator's `morning-dashboard-web.template.html` does. Nothing in this build routes
to it — no skill step publishes it, and it takes four placeholders rather than the desktop
template's set. It ships so the work is not lost, not because it is ready. Do NOT publish it
as though it were the board, and do NOT treat its presence as evidence that a web pipeline
board exists. Build the board from `pipeline-board.template.html` as described above.
...
candidate queued by `zoxiro-admin`'s deal-flow scan (or the scheduled Daily Inbox
Review template), not waiting for a full underwriting verdict. `zoxiro-underwriting`'s
completion step (and the scheduled Daily Underwriting Run) is the fallback trigger
for deals that reach a verdict without ever passing through that queue. You
...
- **Underwrite a deal in the pipeline** → **zoxiro-underwriting**. STR /
  vacation-rental deals are not supported in this build — say so plainly and offer to
  underwrite it as a long-term rental or just track it in the pipeline.
